...
{ Como no sabes la cantidad de cobradores necesitas un mientras (while)
...
// También necesitas un mientras para controlar los clientes
	
			While Clientes <> '' do
				Begin
					TotalClientesCob:=TotalClientesCob+1;

 // Con un repetir controlas que solo ingrese las opciones que queres,
						Repeat  
							Writeln('Ingrese el plan (A o B)');
							Readln (Plan);	
							Plan:= Upcase (Plan);
				...
				End;
			ClrScr;
			Writeln ('Resumen del Cobrador: ', Cobrador); // Cuando hay mas clientes del cobrador te muestra un resumen
			...
			If CuotaCobradorMax< CuotaCobrador then // aca guardamos que cobrador cobro mas
				...
				End;
	// aca abajo acumular la información para los totales de la empresa		
			CuotaEmpresa:=CuotaEmpresa+CuotaCobrador;
	...
	// aca abajo se ponen los contadores a cero para el proximo cobrador
			ContClientesA:=0;
			ContClientesB:=0;
			TotalClientesCob:=0;
			CuotaCobrador:=0;
		...
		End;
	// cuando no hay mas cobradores muestra el resumen de la empresa

	Writeln ('El total cobrado por la Empresa: ', CuotaEmpresa:2:2);
